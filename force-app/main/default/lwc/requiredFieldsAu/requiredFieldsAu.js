...
import LEAD_LEAD_SOURCE_DETAILS_FIELD from '@salesforce/schema/Lead.LeadSourceDetails__c';
...
const fields = [//LEAD_AWARENESS_SOURCE_FIELD,
                ...
                LEAD_LEAD_SOURCE_DETAILS_FIELD
               ]
...
export default class RequiredFieldsLwc extends LightningElement {
    ...
    leadSource = LEAD_LEAD_SOURCE_FIELD;
    leadSourceDetails = LEAD_LEAD_SOURCE_DETAILS_FIELD;
    ...
    @track leadSourceDetailSwitch = false;
    @track init = 'init';
    ...
    renderedCallback() {
        ...
        this.leadSourceSwitchStart(this.init);
    }
        ...
        const switchArray =
            {
                ...
                leadSource: this.leadSourceSwitch(),
                leadSourceDetails: this.leadSourceSwitch()
            };
        return switchArray;
    }
    ...
    leadSourceSwitch() {
        let sourceSwitch;
        if ((getFieldValue(this.record.data, LEAD_LEAD_SOURCE_FIELD) == 'Other') &&
            !getFieldValue(this.record.data, LEAD_LEAD_SOURCE_DETAILS_FIELD)) {
                sourceSwitch = false;
            } else if ((getFieldValue(this.record.data, LEAD_LEAD_SOURCE_FIELD) == 'Other') &&
                       getFieldValue(this.record.data, LEAD_LEAD_SOURCE_DETAILS_FIELD)) {
                sourceSwitch = true;
            } else if (getFieldValue(this.record.data, LEAD_LEAD_SOURCE_FIELD)) {
                sourceSwitch = true;
            } else {
                sourceSwitch = false;
            }
    ...
    get visibleSwitch() {
        let visSwitch;
             if (//getFieldValue(this.record.data, LEAD_AWARENESS_SOURCE_FIELD) &&
                 ...
                 this.consumptionSwitch() && this.phoneSwitch() &&
                 this.hasPvPlantAllSwitch() && this.otherOfferDetailsSwitch() &&
                 this.addressSwitch() && this.leadSourceSwitch() ) {
                    visSwitch = true;
                 } else {
                    visSwitch = false;
                 }
    ...
    leadSourceChange(event) {
        ...
        if (event.target.value == 'Other') {
            this.leadSourceSwitchStart('change_Other');
        } else {
            this.leadSourceSwitchStart('change_Else');
        }
    }

    leadSourceSwitchStart(event) {
        if (event == 'change_Other') {
            this.leadSourceDetailSwitch = true;
        } else if (event == 'change_Else') {
            this.leadSourceDetailSwitch = false;
        } else if (event == 'init') {
            if (this.leadSourceSwitch()) {
                this.leadSourceDetailSwitch = true;
            }
        }
    }
